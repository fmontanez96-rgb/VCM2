...
function onYouTubeIframeAPIReady() {
    ytPlayer = new YT.Player('youtube-player', {
        ...
        playerVars: { 
            ...
            'playsinline': 1,
            'enablejsapi': 1,
            'origin': window.location.origin // Esto le dice a YouTube: "confía en quien me esté sirviendo"
        },
        events: {
            'onReady': (event) => {
...
function closeEventoModal() {
    document.getElementById('modal-agregar-evento').classList.remove('active');
}
...
function guardarEvento() {
    ...
    if (tipo === 'viaje-salida') {
        ...
        const slotCheckOut = parseInt(document.getElementById('hospedaje-checkout-hora').value) || 10;
